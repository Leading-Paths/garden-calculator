...
export default function CalculationResults() {
  ...
  }, [gardenData.dimensions, gardenData.measurementPoints, gardenData.sections]);
    ...
    <div className='space-y-4 p-4 bg-white rounded-lg shadow'>
      ...
      <div className='grid grid-cols-2 gap-4'>
        <div className='p-3 bg-blue-50 rounded'>
          ...
          <p className='text-xl font-bold text-blue-700'>{formatArea(calculations.totalArea, gardenData.dimensions.unit)}</p>
        </div>

        <div className='p-3 bg-green-50 rounded'>
          <p className='text-sm text-gray-600'>Perimeter</p>
          <p className='text-xl font-bold text-green-700'>{formatDistance(calculations.perimeter, gardenData.dimensions.unit)}</p>
        </div>
      </div>
          ...
          <div className='space-y-2'>
            {gardenData.sections.map((section) => (
              <div key={section.id} className='flex justify-between items-center p-2 bg-gray-50 rounded'>
                ...
                <span className='text-sm text-gray-600'>{formatArea(calculations.sectionAreas[section.id] || 0, gardenData.dimensions.unit)}</span>
              </div>
            ))}
          </div>
        </div>
